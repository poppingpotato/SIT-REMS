<script>
    $(document).ready(function(){
        ...
        $('#btnAdd').click(function(){
            ...
            $('#headerAddUpdateModal').html("Add New Student");
            $('#btnSave').val('Create');
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if(buttonType == "Create"){
                        ...
                        swal("Success", "Student record was created successfully!", "success");
                        
                    }
                    else if (buttonType == "Update"){
                        $('#record_id_' + data.id).replaceWith(newData); //updating the data without refresh
                        swal("Success", "Student details was updated successfully!", "success");
                    }
                },
        ...
        $('body').on('click', '#btnEdit', function(){
            $('#addUpdateModal').modal('show');
            $('#headerAddUpdateModal').html("Update Student Details");
            $('#btnSave').val('Update');
            $('#addUpdateForm').trigger("reset");

            data_id = $(this).attr('data-id');
            
            $.get("{{ route('student.index') }}" + '/' + data_id + '/edit', function(data){ //gets the fields of data using the id 
                console.log(data.id)
                $('#id').val(data.id);
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        ...
                        $.ajax({
                        type: "DELETE", 
                        url: "{{ route('student.index')}}" + '/' + data_id,
                            
                        success: function(){
                            swal("Success", "Goodbye Student Data", "success");
